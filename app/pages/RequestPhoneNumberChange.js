...
const RequestPhoneNumberChange = ({ route, navigation }) => {
  ...
  const onPhoneNumberChange = () => {
    ...
    if (phoneNumber === null || phoneNumber === '') {
      // Change 1: Use Alert instead of ErrorView for empty phone number
      Alert.alert(
        "Validation Error",
    ...
    CommonService.commonPost(navigation, url, payLoadData).then(async (data) => {
      if (data !== undefined) {
        setErrorViewConfig({
          message: 'The request of Phone number change is received, our team will get back to you shortly.',
      ...
      {isLoading ? (
        ...
        <View style={styles.container}>
          ...
          <View style={[styles.headerContainer, common_styles.borderBottom]}>
            <Text style={styles.headerText}>Phone number change request</Text>
            <Icon
          ...
          >
            {oldPhoneNumber &&
              <View style={{ paddingTop: 15, paddingLeft: 20, paddingRight: 20, marginBottom: 20 }}>
...
export default RequestPhoneNumberChange;