...
const RequestChitPledgeRelease = ({ route, navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      await getDropdownOptions('CHITPLEDGERELEASE');
    } catch (err) {
      setErrorViewConfig({
  ...
  const getDropdownOptions = async (type) => {
    try {
      ...
      if (!response || response.length === 0) {
        setErrorViewConfig({
          message: 'There are no chits available for pledge release.',
  ...
  const onTransferChit = async () => {
    try {
      ...
      if (selectedGroupName === null || selectedGroupName === '') {
        Alert.alert(
          "Validation Error",
          "Pledge release chit number cannot be empty",
      ...
      if (data !== undefined) {
        ...
        setErrorViewConfig({
          message: 'Chit pledge request is submitted successfully.',
  ...
  return (
    <View style={[styles.container, insets.bottom > 20 ? { paddingBottom: insets.bottom } : {}]}>
      ...
      <View style={[styles.headerContainer, common_styles.borderBottom]}>
        <Text style={styles.headerText}>Chit pledge release request</Text>
...
export default RequestChitPledgeRelease;
