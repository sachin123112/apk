...
const RequestChitCancel = ({ route, navigation }) => {
  ...
  const loadData = async () => {
    try {
      ...
      await getDropdownOptions('CancelChit');
    } catch (err) {
      setErrorViewConfig({
  ...
  const getDropdownOptions = async (type) => {
    try {
      ...
      if (!response || response.length === 0) {
        setErrorViewConfig({
          message: 'There are no chits available to cancel.',
  ...
  const onCancelChit = async () => {
    try {
      ...
      if (data !== undefined) {
        ...
        setErrorViewConfig({
          message: 'Cancel chit request is submitted successfully.',
  ...
  return (
    <View style={[styles.container, insets.bottom > 20 ? { paddingBottom: insets.bottom } : {}]}>
      ...
      <View style={[styles.headerContainer, common_styles.borderBottom]}>
        <Text style={styles.headerText}>Cancel chit request</Text>
        <Icon
...
export default RequestChitCancel;
